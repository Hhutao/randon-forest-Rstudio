...
# 加载数据
data3 <- read.csv("wh_selected_10.csv")
colnames(data3)
for (i in c(1,3:5,7)) {
...
for (i in 1:10) {
  if (i %in% c(1,3:5,7)) {
    ggplot(data3, aes_string(x = colnames(data3)[i])) +
      ...
      theme_bw()
    ggsave(paste0("wh_", colnames(data3)[i], ".png"))
  } else {
    ggplot(data3, aes_string(x = colnames(data3)[i])) +
...
ggsave("wh_all_corplot.tiff", height = 10, width = 10)
DataExplorer::plot_correlation(data3, type = "continuous")
ggsave("wh_cont_corplot.tiff", height = 10, width = 10)
DataExplorer::plot_correlation(data3, type = "discrete")
ggsave("wh_disc_corplot.tiff", height = 10, width = 10)
...
write.csv(rbind(evaltrain, evaltest), "wh_eval.csv", row.names = F)
...
testdata2 <- testdata %>%
  ...
  mutate(预测误差 = 均价 - 预测均价,
             相对绝对误差 = abs(预测误差)/均价)
write.csv(testdata2, "wh_predtest.csv", row.names = F)
ggplot(testdata2, aes(x = 均价, y = 预测均价)) +
  geom_point() +
  geom_smooth(method = lm, se = F) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  theme_bw()
ggsave("wh_predtest.tiff", height = 10, width = 10)
